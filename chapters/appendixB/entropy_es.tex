...
la siguiente suposición moderada sobre la variable aleatoria que representa
...
Obsérvese que esta suposición se cumple para (casi) todas las variables que nos interesan
...
diferencial de una variable aleatoria \(\vx\) que no tiene densidad
...
En este caso, \(\vx\) no tendría densidad; en el mundo contrafactual
...
Con este fin, la idea básica consiste en tomar un engrosamiento de espesor \(\eps\) de
...
\begin{figure}[th]
    ...
    \end{tikzpicture}
    \caption{Ilustración del engrosamiento de espesor \(\eps\) \(\cS_\eps\) de
...
\(\cS_{\eps}\) tiene volumen positivo, \(\vx_{\eps}\) tiene una densidad
\(p_{\eps}\) igual a
\begin{equation}
...
\begin{theorem}\label{thm:max_entropy}
    ...
    \begin{enumerate}
        \item Si \(\vx\) tiene soporte en un conjunto compacto \(\cS
            ...
            \end{equation}
        \item Si \(\vx\) tiene covarianza finita tal que, para una matriz PSD
...
variable aleatoria de Bernoulli, que no tiene densidad), su
...
Sin embargo, también debemos añadir una suposición sobre la
...
\begin{proof}
    ...
    \(h(\vx_{t})\), que, al fin y al cabo, no es más que una funcional de \(p_{t}\).
    ...
    no se cumpliera, \(\nabla p_{t}(\vxi)\) tendría que desaparecer casi
    en todas partes (es decir, en todas partes excepto, posiblemente, en un conjunto de volumen), 
...
Antes de ponernos manos a la obra, haremos varias observaciones sobre la
formulación del problema. En primer lugar, la fórmula de Tweedie \eqref{eq:tweedie} establece que
\begin{equation}
...
\begin{proof}
    ...
    Ahora ponemos en práctica estas ideas. A partir de \Cref{cor:gribonval_A2},
    ...
    es la jacobiana de \(\bar{\vx}\). Dado que por el
...
Los pasos deben volverse cada vez más finos a medida que \(t \to 0\), una
...
intermedios}\label{sub:app_diffusion_intermediate_results}

En esta subsección presentamos los resultados técnicos que sustentan nuestros dos
teoremas conceptuales principales. Nuestra presentación seguirá más o menos
el enfoque habitual en matemáticas; comenzaremos primero con los resultados de mayor nivel
y, poco a poco, iremos retrocediendo hacia los resultados más incrementales. Los
resultados de mayor nivel utilizarán los resultados incrementales, y de esta
manera obtenemos un orden de dependencia de los resultados fácil de seguir: ningún
...
\end{lemma}
\begin{proof}
    Para demostrar el \Cref{lem:diffusion_entropy_exists}.1, utilizamos un argumento analítico clásico
    pero tedioso. Dado que \(\vx_{t}\) tiene una densidad, podemos escribir
    \begin{equation}
        h(\vx_{t}) = -\int_{\R^{D}}p_{t}(\vxi)\log p_{t}(\vxi) \odif{\vxi}.
    \end{equation}
    Por consiguiente, sea \(g \colon \R^{D} \to \R\) definida como
    \begin{equation}
...
El argumento es conceptualmente bastante sencillo, pero requiere algunas
estimaciones técnicas para demostrar que el término de frontera en la
integración por partes se anula.
...
\begin{lemma}[Generalización de \cite{Gribonval2011-pf}, Lema
    ...
    \(\bar{\vx}(\vxi) \doteq \Ex[\vx_{s} \mid \vx_{t} = \vxi]\). La
    jacobiana \(\bar{\vx}^{\prime}(\vxi)\) es simétrica y definida positiva.
\end{lemma}
\begin{proof}
    Tenemos
    \begin{equation}
    ...
    la suposición (es decir, que \(\vx\) tiene densidad en \(\R^{D}\)), por lo que
    la jacobiana \(\bar{\vx}^{\prime}(\vxi)\) es simétrica y definida positiva.
\end{proof}
...
\begin{lemma}[Generalización de \cite{Gribonval2011-pf} Corolario
    ...
    cuya jacobiana \(f^{\prime}(\vx)\) sea simétrica y positiva
    definida. Entonces \(f\) es inyectiva y, por lo tanto, invertible como
...
\begin{proof}
    ...
    \begin{equation}
        ...
        t^{\ast}\vv)}\vv > 0
    \end{equation}
    ya que la jacobiana es definida positiva. Por lo tanto, llegamos a una
    contradicción, tal como se ha afirmado.
\end{proof}

Al combinar los dos resultados anteriores, obtenemos el siguiente resultado fundamental.
...
Por último, elaboramos una estimación técnica necesaria para la
demostración del \Cref{thm:conditioning_reduces_entropy} y que, de hecho,
justifica la suposición relativa a los valores viables de \(t\).
...
Aquí calculamos algunas derivadas útiles que se volverán a utilizar
a lo largo del apéndice.
...
veces bajo el signo de la integral, y es importante saber cuándo podemos hacerlo. Hay dos
...
En cuanto a la primera categoría, ofrecemos un resultado concreto, expuesto sin
demostración pero atribuible a
\href{https://planetmath.org/differentiationundertheintegralsign}{la
fuente enlazada}, que deduce el siguiente resultado como un caso especial
...
\begin{proposition}[\cite{jones1982theory}, Sección 11.12]\label{prop:dutis}
    ...
    \begin{itemize}
        ...
        \item Para casi todo \(\vxi \in \R^{D}\) según Lebesgue, la
...
Tal como se ha adelantado, haremos una suposición de regularidad sobre el conjunto de soporte $\cS$
...
fractal). Sin embargo, hemos constatado que calcular con precisión las constantes en estos
resultados---una tarea necesaria para demostrar una conclusión como el
\Cref{thm:covering-number-rate-distortion}---resulta algo laborioso en nuestro
contexto.
Por lo tanto, nuestro enfoque consiste en añadir una suposición de regularidad geométrica sobre
el conjunto $\cS$ que sacrifica cierta generalidad, pero nos permite desarrollar
...
direcciones principales. Cuando las dimensiones de los componentes $d_k$ son grandes, esta
suposición equivale a una suposición de mezcla de gaussianas de bajo rango, debido a
...
establecidas de manera compatible con esta suposición.
...
La suposición de que $\vx$ sigue una distribución uniforme en su soporte se formaliza fácilmente
...
\begin{definition}\label{def:thickening-set}
    ...
    Para cualquier $\delta > 0$, definimos el engrosamiento de $\cS$ de espesor $\delta$, denotado
    $\cS_{\delta}$, como
    \begin{equation}
...
\begin{definition}\label{def:thickening-rv-uos}
    ...
    $\cS_{k, \delta}$ (\Cref{def:conjunto-engrosado}), para $k \in [K]$, con pesos de mezcla
    $\pi_k$.
\end{definition}
...
\begin{proof}[(Demostración de \Cref{lem:rate-distortion-lb-uos})]
    ...
    una distorsión no mucho mayor, si se elige un valor adecuado de $\delta$.
    ...
    \begin{align}
        ...
        \leq \delta,
    \end{align}
    donde la segunda línea se basa en la suposición de ortogonalidad de los subespacios
    ...
    a un único subespacio $\vU_k$, lo que significa que los componentes de la mezcla
    ...
    vemos que necesitamos «integrar» sobre los componentes $r$ y $\vxi^\perp$
    ...
    uniforme.
    Dado que la suposición sobre $\delta$ implica que los componentes de la mezcla en
