...
因此，步长 \(h\) 应该根据函数 \(\cL(\theta_{k})\) 的全景（landscape）来选择。理想情况下，为了选择最佳步长 \(h\)，我们可以求解以下关于单一变量 \(h\) 的优化问题：
\begin{equation}
...
那么我们应该如何选择合适的步长 \(h\) 呢？一种常见且经典的方法是，尝试基于对函数 \(\cL(\theta)\) 整体全景的一些了解，获得当前状态 \(\theta\) 附近局部全景的良好近似。
...
\end{equation}
这意味着步长 \(h = 1/\beta\) 是一个可用的学习率，但它并没有提供收敛速度，也没有证明 \(L(\theta_{k})\) 实际上收敛于 \(\min_{\theta}\cL(\theta)\)。这需要稍微更严格的论证，我们现在就来进行。
...
\begin{align}
    ...
    &= \bp{1 - \alpha h}\norm{\theta^{\star} - \theta_{k}}_{2}^{2} +
    2h(\cL(\theta^{\star}) - L(\theta_{k})) + h^{2}\norm{\nabla
    \cL(\theta_{k})}_{2}^{2} \\
    &\leq \bp{1 - \alpha h}\norm{\theta^{\star} - \theta_{k}}_{2}^{2}
    + 2h(\cL(\theta^{k}) - \cL(\theta^{\star})) +
...
= 1}\xi_{i}\)，但随机梯度下降可能会在全局最优值附近“像弹球一样来回震荡（pinball）”，而不会收敛，如\Cref{fig:sgd_nonconvergence}所示。
...
\end{equation}
现在考虑随机情况，即我们在每次迭代中优化不同的损失 \(L_{k}\)。在 SGD 中，我们使用动量“跟踪”（即取平均）梯度。在这里，我们可以使用动量同时跟踪梯度和平方梯度，即
\begin{align}
    \vg_{k}
...
\dots, \theta^{m}, \theta^{\head})\) 且特定任务头为 \(h_{\theta^{\head}}\)，并记
\begin{align}
...
\begin{example}
    ...
    \begin{align}
        ...
        \odif{\vb}^{\ell})\vone^{\top}] - [\vW^{\ell}\vZ + \vb^{\ell}] \\
...
\begin{proof} %
    ...
    因此
    \begin{equation}
        \max_{\theta \in \Theta}\cL(\theta, \eta) \leq
