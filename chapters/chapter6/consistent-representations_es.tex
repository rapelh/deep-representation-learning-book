...
$f(\mathcal{M})$ sea aplanada, como se ilustra en el ejemplo mostrado en la
...
muestreados de manera uniforme de todas las clases al mismo tiempo. Se sabe que
cuando una red (de bucle abierto) se actualiza para aprender nuevas clases sin
datos de las anteriores, el conocimiento adquirido previamente se ve afectado
por el problema del {\em olvido catastrófico}
\cite{McCloskey1989catastrophic}. Esto se conoce en neurociencia como
el dilema estabilidad-plasticidad: el desafío de garantizar que un
...
de las partes utilizadas y no utilizadas de la red---requieren
heurísticas y conllevan costos ocultos.
...
\begin{enumerate}
    \item Lo primero es que debe ser \emph{basado en la memoria}. Al
        ...
        de las clases antiguas, así como las correspondientes correspondencias de 
        codificación y decodificación \cite{fearnet}.
    \item La segunda es que debe ser \emph{autónomo}.
...
dado que la memoria en el cerebro se forma de manera incremental, ¿puede
el marco de transcripción de bucle cerrado mencionado anteriormente también admitir un aprendizaje incremental?
...
autocodificación aceptable, incluso sin información de clase, con el
...
pero en la práctica pueden aplicarse de manera flexible durante la optimización.
...
autocodificación consistente a nivel de muestra, necesitamos lograr que $\hat \x$
se aproxime a $\x$ para cada muestra. En el marco de CTRL, esto se puede
...
lo mejor que podemos hacer es considerar cada muestra
y sus aumentaciones (por ejemplo, mediante traslación, rotación, oclusión,
etc.) como una sola «clase»: una idea básica que subyace a casi todos los métodos de aprendizaje 
autosupervisado. En el marco de la reducción de la tasa, resulta natural
...
facilitar tareas discriminativas o generativas, debe estar
muy estructurada. Se ha comprobado experimentalmente que así es
...
\cite{krizhevsky2014cifar}, utilizando aumentaciones estándar para
el aprendizaje autosupervisado \cite{chen2020simple}. Se puede consultar
...
grupos significativos y modelarlas con distribuciones (gaussianas) de baja
...
realizando un muestreo y generando datos en consecuencia. Aunque los experimentos que aquí se presentan tienen
...
en la investigación sobre redes neuronales para el aprendizaje, aunque las demostraciones más impresionantes
desde el punto de vista práctico del aprendizaje profundo probablemente se hayan dado en otros ámbitos
...
\cite{Hinton504}, sirvieron de catalizadores del interés por la investigación en redes neuronales
en una época en la que, de otro modo, estas no ocupaban un lugar destacado en el panorama de la investigación
...
\cite{van-den-Oord2017-jr}, que se basa en la metodología de los autocodificadores variacionales
...
debido a su estrecha relación con el aprendizaje de representaciones y prevemos que
volverá a cobrar relevancia para los investigadores aplicados en el futuro, a medida que
...
\cite{pai2022pursuit} ha aportado sólidas justificaciones teóricas
...
capa oculta (y no linealidades adecuadas en cada elemento) es suficiente para
aproximar cualquier función objetivo adecuadamente regular. Por supuesto, la
principal razón arquitectónica del predominio de las redes neuronales en la práctica ha
sido el perfeccionamiento de las técnicas para entrenar redes neuronales \textit{más profundas}.
¿Por qué es necesaria la profundidad?
Desde un punto de vista fundamental, la cuestión de las separaciones de profundidad, que
permite establecer escenarios en los que una red neuronal más profunda puede aproximarse a una clase determinada
de funciones objetivo con una eficiencia exponencialmente superior en comparación con una red
superficial,
...
la capacidad de entrenamiento de las redes muy profundas, cuantificada a través del núcleo tangencial neuronal inicial
...
una superficie curva en el espacio tridimensional), tal y como se explica en la subsección sobre
aplanamiento de variedades de la \Cref{sub:nonlinear-pca}.
En este ejercicio, describiremos los componentes básicos del algoritmo de
aplanamiento de variedades de \cite{Psenka-JMLR24}.
Una variedad se denomina plana si es un conjunto abierto en el espacio euclídeo (o,
...
\item Supongamos que la variedad $\cM$ es un \textit{grafo}: esto significa que $\cM
    ...
    \end{equation}
    Fije un número entero $D_3$ y una aplicación $f : \R^{D_1} \times \R^{D_2} \to \R^{D_3}$
    que aplane $\cM$ y describa el procedimiento de reconstrucción (sin pérdida de información)
    correspondiente a partir de la representación aplanada.
\item Supongamos ahora que $\cM$ es una variedad general suave. Las variedades suaves
    ...
    cerca de cada punto. Describe en términos intuitivos cómo aplanar la variedad
...
Consulte \cite{Dai-entropy-2022} para conocer los hiperparámetros y los ajustes de arquitectura
