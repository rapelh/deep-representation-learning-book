...
\subsubsection{De Brujin恒等式中的分部积分}
...
\begin{proof}
    ...
    \begin{align}
        ...
        p_{t}(\vxi)}{\nabla p_{t}(\vxi)}}\odif{\vxi} \\
        &= \lim_{r \to \infty}\int_{\partial B_{r}(\vzero)}\bc{\Delta
            p_{t}(\vxi)[c + \log p_{t}(\vxi)] + \ip{\nabla \log
        p_{t}(\vxi)}{\nabla p_{t}(\vxi)}}\odif{\vxi} \\
...
\begin{lemma}\label{lem:app_diffusion_laplacian_control}
    ...
    \begin{equation}
        ...
        \max\rp{\frac{D}{t^{2}}, \abs*{\frac{R}{t^{4}} - \frac{D}{t^{2}}}}.
    \end{equation}
\end{lemma}
\begin{proof}
    根据链式法则，通过简单的计算可得
    ...
    \end{align}
    该迹的一个平凡下界是 \(0\)，因为协方差矩阵是半正定的。为了寻找上界，注意到 \(\vy_{\vxi}\) 仅在 \(\vx\) 的支撑集上取值（因为 \(p\) 是 \(\vy_{\vxi}\) 的密度 \(q_{\vxi}\) 的一个因子），根据 \Cref{assumption:entropy_x_compact_support}，该支撑集是一个紧集 \(\cS\)，其半径为 \(R \doteq \sup_{\vxi \in \R^{D}}\norm{\vxi}_{2}\)。因此
    \begin{equation}
        \tr(\Cov(\vy_{\vxi})) = \Ex[\norm{\vy_{\vxi}}_{2}^{2}] -
