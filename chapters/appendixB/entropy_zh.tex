...
\begin{theorem}\label{thm:conditioning_reduces_entropy}
    设\(\vx\)为满足\Cref{assumption:entropy_x_compact_support,assumption:entropy_x_density}的任意随机变量，并设\((\vx_{t})_{t \in [0, T]}\)为随机过程\eqref{eq:app_additive_gaussian_noise_model}。对每个 \(t > 0\)，记
    \begin{equation}
        J(p_{t}) \doteq
        \int_{\R^{D}}\frac{\norm{\nabla
    ...
    \end{equation}
    分别表示 \(p_t\) 的 Fisher 信息以及 \(\abs{\Delta \log p_t}\) 的一致上界。那么
    \begin{equation}
        h(\Ex[\vx_{s} \mid \vx_{t}]) < h(\vx_{t})
    \end{equation}
    对所有满足如下条件的 \(0 \leq s < t \leq T\) 成立：
...
因此，我们考虑贯穿本书的运行示例，即低秩高斯分布的混合。在这种几何设定下，我们将 $\cS$ 建模为若干超球面的并集，每个超球面的维度为 \(d_k\)（可能远小于 \(D\)），并位于 \(\R^D\) 中相互正交的子空间里。这是对高斯混合模型的一种几何简化：每个分量的支撑集由其主方向张成的子空间中的一个球面近似。当分量维度 \(d_k\) 较大时，由高维测度集中性可知，该假设等价于低秩高斯混合假设。我们在\Cref{ch:deep-networks,ch:applications}中开发和训练的 CRATE 模型，其\textit{子空间维度}的设置与这一假设相容。
...
\subsection{失真编码率与覆盖之间关系的证明}

我们简要概述证明思路，然后着手建立三个基本引理，最后给出证明。该证明将依赖于下面概述中引入的概念。

获得失真编码率函数 \eqref{eqn:rate-distortion-general} 的上界是直接的：根据编码率的刻画（即，失真编码率函数是在 $\vx$ 的期望平方 $\ell^2$ 失真为 $\epsilon$ 时，对 $\vx$ 进行编码所需的最小编码率），对 $\cR_{\epsilon}(\x)$ 求上界只需要展示一种达到该目标失真的 $\x$ 的编码，而 $\Supp(\x)$ 的任何 $\epsilon$-覆盖都能实现这一点，其编码率等于该覆盖基数的以 $2$ 为底的对数。
