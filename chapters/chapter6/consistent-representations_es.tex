...
\chapter{Representaciones consistentes y auto-consistentes}
...
\begin{equation}
    d(\X, \hat \X).
\end{equation}
Esto nos lleva al concepto de {\em representación consistente}.\footnote{Téngase en cuenta que, 
en este contexto, el concepto de consistencia es de naturaleza empírica o inductiva. Es 
diferente del concepto de consistencia lógica y deductiva en matemáticas, también conocido como 
el Principio de Hilbert, del cual hablaremos con más detalle en el capítulo \ref{ch:future}. } Como
ya mencionamos brevemente en el
\Cref{ch:intro} (véase la \Cref{fig:autoencoder}), la {\em auto-codificación} ---que integra los
...
extender el auto-codificador a clases más generales de distribuciones cuyos
...
\begin{figure}
    ...
    \caption{Aprender una buena representación de auto-codificación para una distribución general
...
Dado este formalismo clásico de auto-codificación, surge una pregunta natural:
...
donde el marco del «auto-codificador de representaciones» muestra cómo las representaciones aprendidas en bucle abierto
...
Esto nos lleva al concepto de una {\em representación auto-consistente}.
En la \Cref{sec:self-consistency}, estudiaremos cuándo y cómo podemos
aprender una representación consistente imponiendo la auto-consistencia
...
representación auto-consistente en un entorno de {\em aprendizaje continuo}.
...
de inteligencia, como la clasificación, el completamiento y la predicción.
...
A continuación ofrecemos una definición formal de representaciones consistentes, que
están estrechamente relacionadas con el concepto de auto-codificación. %
\begin{definition}[Representaciones consistentes]\label{def:bidirectional_rep}
    ...
    estructuradas y la \textit{auto-codificación}
    \begin{equation}
...
auto-codificación es garantizar que el $\Z$ así obtenido sea a la vez más
...
determinada distancia entre distribuciones, como la divergencia KL
...
buscamos una buena auto-codificación para una distribución de datos
...
\end{equation}
Observe que el error de cuantización $\epsilon$ actúa como parámetro de compromiso 
en esta relajación lagrangiana.

En el resto de esta sección, estudiaremos cómo resolver este tipo de
problemas de auto-codificación bajo diferentes condiciones, desde casos simples e
...
\subsection{Auto-codificación lineal mediante PCA}% or Online PCA}

Según \cite{Baldi2011}, el término «autoencoder» fue
...
manera auto-supervisada; la reconstrucción de los datos originales constituye la
tarea de auto-supervisión. Sin embargo, el concepto mismo de buscar una
...
\begin{figure}
    ...
    \caption{Ilustración de un auto-codificador típico, como el PCA, que busca
...
auto-codificación}\label{sub:nonlinear-pca}\label{sec:NLPCA}
...
En general, el problema de aprender un mapeo de auto-codificación de este tipo para esta clase
de distribuciones de datos se conoce como {\em análisis no lineal de componentes principales}
(NLPCA).
...
\begin{figure}[tb]
    ...
    \caption{PCA no lineal mediante redes neuronales auto-asociativas de dos capas
...
es posible aprender auto-codificadores consistentes mediante este marco ---puesto que, para
cualquier muestra aleatoria de datos, siempre que se disponga de suficientes parámetros, existen tales pares de 
auto-codificadores--- encontrarlos mediante descenso por gradiente muchas veces resulta extremadamte no trivial.
...
en forma heurística.\footnote{En el trabajo posterior \cite{Hinton-1993}, Hinton
...
debido a que el gradiente de la función sigmoide se desvanece. Por lo tanto, la
...
los auto-codificadores más profundos a los menos profundos.
...
flexibles para la auto-codificación incremental.

\subsection{Auto-codificación dispersa}
En los esquemas de auto-codificación mencionados anteriormente, la dimensión del espacio de características
...
la auto-codificación suele realizarse en forma empírica. En situaciones más generales,
...
El auto-codificador disperso está diseñado para resolver algunas de estas limitaciones. En
...
\Cref{ch:deep-networks} para derivar la arquitectura CRATE transparente white-box.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{\toplevelprefix/chapters/chapter6/figs/SAE_diagram.png}
    \caption{Ilustración de un auto-codificador disperso (SAE), en comparación con
    la de un auto-codificador típico (AE) en la \Cref{fig:AE}. }
    \label{fig:SAE}
\end{figure}

Como método para aprender pares de auto-codificación en forma integral, la auto-codificación
dispersa se ha utilizado en el pasado
\cite{Ranzato2006-oq,10.5555/3042573.3042641}, pero casi todos los marcos modernos
de auto-codificación se basan, en cambio, en un marco de auto-codificación probabilístico
diferente, que estudiaremos a continuación.

\subsection{Auto-codificación variacional}\label{sec:vae}
...
Una metodología mejorada y más moderna para la auto-codificación que sigue teniendo
una aplicación significativa en la actualidad es la \textit{auto-codificación variacional}
...
distribución poblacional de datos):
\begin{equation*}
...
Mediante un condicionamiento, podemos escribir $p(\vx, \vz ;\, \vtheta)
...
corresponden al \textit{decodificador} del auto-codificador.
...
codificador MLP $f = (f_1, f_2)$ con parámetros $\veta$:
\begin{equation*}
...
\end{align*}
Podemos interpretar esto como un objetivo de auto-codificación regularizado, en el que existe
...
\begin{figure}[t]
    ...
    \caption{Diagrama del modelo de difusión latente tomado del trabajo de
        \cite{rombach2022high}, que utiliza un auto-codificador variacional
        ...
        de la derecha trata de cómo controlar el proceso de muestreo para la eliminación de ruido
...
complicado debido a problemas como el colapso de la distribución a posteriori y el equilibrio
...
Estos desafíos han impulsado el desarrollo de enfoques alternativos,
como los auto-codificadores de representación (RAE), que se
...
Los enfoques anteriores para entrenar pares de auto-codificadores con el fin de aprender representaciones
...
la dispersión en el auto-codificador disperso o la gaussianidad en el VAE).
...
\begin{equation}
    f: \X \rightarrow \Z,
\end{equation}
basadas en algunos métodos supervisados o auto-supervisados de extremo a extremo, incluidos los
...
configuraciones clásicas de auto-codificación, cabría esperar que un objetivo de la forma
...
proceso (aprendido) de eliminación de ruido aplicado a un ruido aleatorio:
\begin{equation}
    h: \bm{G} \rightarrow \Z.
\end{equation}
...
\begin{figure}[t]
    ...
    \caption{Ilustración del aprendizaje de una representación auto-codificada
        $\Z$ para los datos $\X$ (arriba) y un modelo de difusión/eliminación de ruido
    (abajo) para acceder a la representación $\Z$.}
    \label{fig:RAE}
\end{figure}

Por lo tanto, en definitiva, queremos entrenar un auto-codificador para los datos $\X$
...
un filtro de ruido $\bar{\z}$ que elimine el ruido en forma iterativa en una muestra, pasando de la
...
no se modela ninguna distribución multivariada del espacio latente, lo que hace que el
...
un \textit{auto-codificador de representaciones} (RAE) con una distribución de representaciones accesible.
...
\section{Aprendizaje de representaciones auto-consistentes}
...
representación consistente, es necesario aprender una auto-codificación bidireccional:
\begin{equation}
    \X
    \xrightarrow{\hspace{1mm} \mathcal{E} = f \hspace{1mm}} \Z
    \xrightarrow{\hspace{1mm} \mathcal{D} = g \hspace{1mm}} \hat{\X}.
...
la distancia de Wasserstein solo pueden calcularse en forma aproximada entre dos
...
distribuciones gaussianas.\footnote{La distancia de Wasserstein (con norma $\ell^1$) puede
    acotarse por la distancia de Wasserstein (con norma $\ell^2$), que tiene una
...
\end{equation}
Esto nos lleva al concepto de {\em representación auto-consistente}.
\begin{definition}[Representación auto-consistente]\label{def:closed_loop}
    Dados los datos \(\vX\), llamamos \textit{representación auto-consistente} 
    a un par de funciones \((f \colon \cX \to \cZ, g \colon \cZ \to \cX)\), 
    de tal manera que las \textit{características} \(\vZ = f(\vX)\) sean 
    compactas y estructuradas y las \textit{características de auto-codificación}
    \(\hat{\vZ} \doteq f \circ g(\vZ)\) sean \textit{cercanas} a \(\vZ\).
    \begin{enumerate}
        \item Decimos que es auto-consistente \textit{por muestra}
            si \(\vZ \approx \hat{\vZ}\) en una determinada norma con alta
            probabilidad.
        \item Decimos que la representación es
            \textit{auto-consistente desde el punto de vista de la distribución} si \(\Law(\vZ)
            \approx \Law(\hat{\vZ})\).
    \end{enumerate}
\end{definition}
...
¿Cómo intentamos garantizar que una representación aprendida sea auto-consistente?
...
auto-codificador, que es lo que hemos estudiado en profundidad anteriormente en este capítulo.
...
No obstante, en este caso el codificador discriminativo $f$ se generaliza en forma natural
...
Obviamente, para garantizar que la auto-codificación aprendida sea auto-consistente, el
...
\begin{example}
    ...
    $g\circ f$ no es necesariamente un mapeo de auto-codificación para $\x$ en la
...
medida de distancia, vemos los roles del codificador $f(\cdot, \bm
\theta)$ y el decodificador $g(\cdot, \bm \eta)$ en forma natural como 
...
siguiente juego de Stackelberg (véase el \Cref{sec:minimax}) para
...
de aprender una buena representación (de auto-codificación) de un conjunto de datos dado. Antes
...
\paragraph{Visualización de la auto-codificación de los datos $\X$ y de los datos decodificados
...
muestras de entrenamiento. Visualmente, el $\hat \x$ auto-codificado captura fielmente
...
como el MNIST, las imágenes generadas mediante auto-codificación son casi idénticas a las originales.
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{Visualización de la propiedad de auto-codificación de la transcripción
...
problema de aprender una distribución de datos como un problema de auto-codificación
...
que la representación \(\vZ_{k}\) y su auto-codificación
...
\begin{theorem}[\cite{pai2022pursuit}, Abridged]\label{thm:ctrl_theory}
    ...
    \begin{itemize}
        ...
        \item La auto-codificación es auto-consistente, es decir, los
            subespacios generados por \(\vZ_{k}\) y \(\hat{\vZ}_{k}\)
            son los mismos para todo \(k\).
    \end{itemize}
\end{theorem}
Este concepto de auto-consistencia es lo máximo que cabe esperar si
...
y la auto-consistencia pueden alcanzar soluciones óptimas.

\section{Aprendizaje continuo de representaciones auto-consistentes}
...
\begin{enumerate}
    ...
    \item Que sea \emph{auto-contenido}.
...
se asemejan mucho a la forma en que se codifican los objetos en diferentes áreas de la
...
puede representarse en forma explícita y concisa mediante un subespacio principal
...
\begin{figure*}[t]
    ...
    \caption{\textbf{Marco general} de nuestro aprendizaje incremental basado en la transcripción
        ...
        $\X_{new}$, se aprende en forma incremental una nueva memoria LDR $\Z_{new}$
...
Obsérvese que, con el auto-codificador aprendido
...
Consideremos la tarea de aprender en forma incremental una nueva clase de
...
«auto-consistente» a través del bucle de transcripción:
\begin{equation}
...
Para emular esta fase en formación de la memoria, tras aprender en forma incremental
...
memoria aprendida en forma conjunta.
...
\paragraph{Visualización de las propiedades de la auto-codificación.}
...
modelo se entrena en forma incremental con los conjuntos de datos divididos en 5 tareas. 
...
muestras individuales, como lo hacen la mayoría de los métodos de auto-codificación, y (2) después de
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{\small Visualización de la propiedad de auto-codificación de la
...
La mayoría de los métodos generativos basados en memoria utilizan auto-codificadores, VAE o
...
las que claramente observamos patrones diagonales en bloques para ambos
...
visualizamos además las componentes principales individuales dentro de cada
uno de esos subespacios. La \Cref{fig:pca_sampling_main} muestra las imágenes
reconstruidas a partir de características muestreadas a lo largo de las cuatro componentes principales
para diferentes clases, en MNIST y CIFAR-10, respectivamente. Cada fila
representa muestras a lo largo de una componente principal y estas muestran claramente
...
de CIFAR-10, la memoria aprendida en forma incremental recuerda posturas y formas representativas
...
\begin{figure}[t]
    ...
    \caption{\small Visualización de 5 $\hat \x=g(\z)$ reconstruidos
        a partir de los valores de $\z$ más cercanos a las (4 principales) componentes
...
Verificamos cómo la memoria LDR aprendida en forma incremental puede consolidarse aún más
...
clases, muestreadas a lo largo de las tres componentes principales principales: cada dos
...
el repaso y las componentes principales de las características en el subespacio
...
auto-codificación aceptable, incluso sin información de clase, mediante el
...
\paragraph{Auto-consistencia muestra por muestra mediante transcripción en bucle cerrado.}
En primer lugar, para abordar el problema de que CTRL-Binary no aprende una
auto-codificación consistente a nivel de muestra, necesitamos promover la proximidad entre $\hat \x$
...
Para promover la auto-consistencia por muestra, donde $\hat{\x} = g(f(\x))$
...
\paragraph{Auto-supervisión mediante la compresión de muestras aumentadas.}
...
auto-supervisado. En el marco de la reducción de la tasa, resulta intuitivo
...
mientras que la auto-consistencia por muestra
...
\begin{figure}[t]
    ...
    método CTRL-Binary: 1) auto-consistencia para las características de cada muestra
...
del aprendizaje auto-supervisado \cite{chen2020simple}. Se puede consultar
...
Como se puede observar en los experimentos, de hecho surge en forma natural una estructura específica y única
...
representa un grupo y cada fila representa una componente principal
...
la generación a partir de las diferentes componentes principales de cada grupo.}
\label{fig:vis_clustering}
\end{figure}

\section{Resumen y Notas}
Históricamente, la auto-codificación ha sido uno de los principales motores de la innovación
...
En la práctica actual, los auto-codificadores siguen siendo componentes fundamentales de muchos sistemas a gran escala
...
\cite{van-den-Oord2017-jr}, que se fundamenta en la metodología de los auto-codificadores variacionales
que analizamos en la \Cref{sec:vae}.
El problema central de la auto-codificación sigue revistiendo una importancia intelectual fundamental
debido a su estrecha vinculación con el aprendizaje de representaciones y anticipamos que
volverá a ser una prioridad para los investigadores prácticos en el futuro, a medida que
...
facilita en forma natural el aprendizaje incremental y continuo, ya sea en un
...
\paragraph{Redes neuronales superficiales vs. profundas, para la auto-codificación y otras aplicaciones.}
...
En este ejercicio, describiremos las componentes básicos del algoritmo de
