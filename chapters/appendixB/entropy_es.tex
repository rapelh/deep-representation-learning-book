...
En particular, dado que los conjuntos compactos en el espacio euclidiano son acotados,
...
\begin{proof}
    ...
    (quizás muy concentrado) \(\vx\) se esparce por todo
    ...
    derivadas de \(p_{t}\) (lo cual hacemos en la
    ...
    \end{equation}
    A continuación, para hallar la dinámica de \(h(\vx_{t})\), podemos volver a utilizar la
...
variable aleatoria \(\vx_{T}\) y eliminamos el ruido en forma iterativa mediante
...
distribución de la variable aleatoria \(\vx_{t}\) por el \textit{campo vectorial de la función de \textit{score}}, 
...
en el \Cref{thm:conditioning_reduces_entropy} \textit{no}
...
\begin{proof}
    ...
    \end{enumerate}
    El cambio de variables se justifica mediante el \Cref{cor:gribonval_A2},
    que se derivó originalmente en \cite{Gribonval2011-pf}.

    A continuación, aplicamos estas ideas. A partir del \Cref{cor:gribonval_A2},
...
Los pasos deben volverse progresivamente más finos a medida que \(t \to 0\), una
...
\begin{proof}
    ...
    contenida dentro de la bola de radio \(R\) y, por lo tanto, el \textit{score}
...
\subsubsection{Invertibilidad local del \textit(denoiser} \(\bar{\vx}\)}
...
discutido en la \Cref{rem:slb}: al calcular las constantes en \cite[\S III,
...
\end{align}
que podemos tomar como el valor explícito de la constante $C_D$ en la \Cref{eq:slb}.
...
nuestra demostración del límite inferior en el \Cref{thm:covering-number-rate-distortion}
...
\begin{proof}[(Demostración de \Cref{lem:rate-distortion-lb-uos})]
    ...
    asociarse en forma única a una proyección sobre un único subespacio de la mezcla,
