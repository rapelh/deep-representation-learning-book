...
la auto-codificación de una manera general, independiente de la distribución o la tarea.
...
En términos generales, una buena representación o auto-codificación debería
...
\begin{example}[Completamiento de imágenes y predicción de texto]
    ...
    GPT) y el completamiento de imágenes (como el auto-codificador enmascarado), que
...
red de auto-codificación se entrena para reconstruir un conjunto de muestras del
...
\subsection{Completamiento de imágenes mediante auto-codificación enmascarada}
...
\begin{figure}[t!]
    ...
    \caption{\small \textbf{Diagrama del proceso general  (enmascarado)
        de auto-codificación.} Las representaciones de los tokens (de imagen) se
...
resolución del siguiente programa de {\em auto-codificación enmascarada} (MAE) que
...
del auto-codificador enmascarado así diseñado. Para obtener más detalles sobre la implementación 
y los resultados del auto-codificador enmascarado para el completamiento de imágenes naturales
...
\begin{figure}[t]
    ...
    \caption{\small \textbf{Visualizaciones de auto-codificación de CRATE-Base
...
El problema de auto-codificación (enmascarada) mencionado anteriormente 
...
consideraremos un escenario más parecido a la auto-codificación enmascarada
...
$p$. Las componentes son los filtros de ruido $\bar{\x}^\ast(t, \vxi) = \bE[ \x \mid
...
\begin{example}\label{example:denoising-conditional-gaussian}
    ...
    filtro de ruido posterior incondicional \eqref{eq:posterior-denoiser-gaussian-case}
...
\begin{figure}[tbp]
    ...
    \caption{Simulación numérica de la configuración de muestreo condicional
        ...
        filtro de ruido posterior condicional exacto como el aproximado
        ...
        filtro de ruido de media a posteriori,
...
y que hemos recordado anteriormente, utilizando el verdadero filtro de ruido posterior condicional, es decir,
la \Cref{eq:conditional-posterior-denoiser-gaussian-case} (las muestras están marcadas con
círculos azules) y la
aproximación conveniente para este filtro de ruido realizada mediante la aproximación de
...
\textcite{rout2023solving} (que utilizan modelos de difusión latente, tal como los hemos
    desarrollado para los auto-codificadores de representación en el
...
En la \Cref{sub:cfg} hemos visto, a nivel conceptual, cómo configurar los filtros de ruido
...
\Cref{ch:consistent-representations} cuando tratamos el tema de la auto-codificación y la transcripción en
...
la observación, se podría construir una arquitectura flexible de filtro de ruido mediante
...
\begin{figure}[tbp]
    ...
    \caption{Esquema general del entrenamiento y la aplicación de un modelo generativo de texto a imagen,
        ...
        para un
        filtro de ruido condicional, entrenado según se describe algorítmicamente en la \Cref{sub:cfg} (véase la
...
de auto-codificadores variacionales especializados y preentrenados $f : \bR^{D_{\mathrm{img}}} \to
...
Obsérvese que, según la definición de la operación de auto-atención, la operación de atención cruzada
...
los bloques residuales de auto-atención de la arquitectura del filtro de ruido, aplicados a la
...
\section{Inferencia condicional con auto-consistencia de la medición}
...
preguntamos si aún podemos desarrollar un filtro de ruido para $\x$ con
...
$\vx$ a través de los filtros de ruido aprendidos $\bar{\vx}_{\theta}(t, \vxi)$. ¿Podemos
aún realizar
...
filtro de ruido a posteriori $\bE[\vx \mid \vx_t=\vxi]$ (una matriz de gran tamaño). Esto se lleva a cabo
