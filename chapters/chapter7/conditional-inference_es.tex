...
$\x$ concreta con una distribución $p(\x)$ que tiene un
...
Si se dispone de una representación aprendida de este tipo, ya es posible utilizarla para realizar
...
del mundo real y que son de alta dimensión, tales como los idiomas naturales, las
...
permitirnos emplear la distribución de baja dimensión aprendida de los
...
\begin{figure}
    ...
    \caption{\small \textbf{Inferencia con distribuciones de baja dimensión
        ...
        o la esperanza condicional \(\mathbb{E}[\vx \mid \y]\), dados
        diversos elementos de información sobre el modelo y (un número potencialmente finito de) muestras 
    de \(\vx\) o de \(\vy\).}
    \label{fig:inference_roadmap}
\end{figure}

\begin{example}[Completamiento de imágenes y predicción de texto]
    La popular tarea de completar imágenes y la predicción de lenguaje natural
...
$$-  \mu (F(\x) - c) \pdv{F}{\vx}(\vx)$$ que siempre apunta
...
sugiere que el «tamaño del paso» de el \textit{score} $\nabla_{\x}
\log p(\x)$ del proceso de eliminación de ruido es 
$$\mu \big| F(\x) - \lambda/\mu\big|$$, 
lo cual, para cualquier $\mu$ fijo, es casi proporcional
...
\begin{figure}
    ...
    \caption{Ilustración del proceso de continuación al aplicar
    las restricciones $F(\x) = 0$ y $\y = h(\x)$.}
    \label{fig:EngMin}
\end{figure}
...
\begin{itemize}
    \item {\em Caso 1:} Se conocen tanto el modelo de distribución de $\x$ como
        ...
        incluso de forma analítica. Este suele ser el
        caso de muchos problemas clásicos de procesamiento de señales, como
        la eliminación de ruido en señales, el problema de recuperación de vectores esparsos que vimos
    ...
    \item {\em Caso 2:} No disponemos de un modelo de distribución,
        ...
        se conoce el modelo de observación $\y = h(\x)$ (posiblemente con ruido añadido 
        $\vw$).\footnote{En la literatura, este escenario se denomina a veces
        ...
        $p(\x\mid \y)$. El completamiento natural de imágenes o 
...
explotarse para tareas de recuperación, completamiento o predicción
...
corrupciones graves y esparsas. Se trata de una clase de problemas que se ha
estudiado ampliamente en la literatura clásica sobre procesamiento de señales,
para estructuras de baja dimensión tales como vectores esparsos, matrices de bajo rango
...
(o incluso estén dañadas). Queremos recuperar o predecir el resto de $\x$
...
conocidas; por ejemplo, de banda limitada, esparsas o de bajo rango. El otro caso es
...
El problema del {\em completamiento de matrices
de rango bajo} es un problema clásico en el ámbito del completamiento de datos cuando
...
es, en general, un problema NP-difícil, nos gustaría relajarlo mediante
...
amplias,\footnote{Por lo general, dichas condiciones especifican la
    cantidad necesaria y suficiente de entradas requeridas para que el
    completamiento sea computacionalmente viable. Estas condiciones han sido
...
(esparsa) y corrupción. Una vez más, los lectores interesados pueden consultar
...
distribución en una forma analítica como la de las matrices de rango bajo, por ejemplo, el
conjunto de todas las imágenes naturales. No obstante, si contamos con suficientes
...
el completamiento y, por lo tanto, para el reconocimiento. Sin embargo, la distribución de todas las
...
\begin{figure}[t!]
    ...
    \caption{\small \textbf{Diagrama del proceso general  (enmascarado)
        ...
        y esparsa) mediante cada capa del codificador \(f^{\ell}\).
...
tal que se maximice la reducción de la tasa y se minimice la esparcidad:
...
Como hemos mostrado en el anterior Capítulo \ref{ch:deep-networks}, el
...
y los resultados del autocodificador enmascarado para el completamiento de imágenes naturales
...
donde $\vA$ es un operador lineal en el espacio matricial\footnote{es decir, si
    imaginamos que desenrollamos \(\vX\) en un vector largo, entonces \(\vA\) desempeña el
...
relacionadas, su distribución multivariada $p(\x, \y)$ es, naturalmente, casi
...
\begin{figure}[t]
    ...
    \caption{Diagramas de dependencia estadística para el proceso de
        ...
        los eliminación de ruido directamente sobre
        ...
        de la
        distribución a posteriori, sino más bien de muestras emparejadas $(\vx, \vy)$ de la distribución multivariada.
        \textbf{Derecha:} Resulta que basta con tener solo
        observaciones ruidosas
        de $\vx$ para realizar los \textit{denoiser} correspondientes a
        ...
        p_{\vx_t \mid
        \vy}$, lo que da lugar a una función de \textit{score} para la eliminación de ruido que consiste en la
        función de \textit{score} incondicional, más un término de corrección que
...
$p$. Las componentes son los textit{denoiser} $\bar{\x}^\ast(t, \vxi) = \bE[ \x \mid
...
realizar la eliminación de ruido iterativa, comenzando desde $\hat{\vx}_{t_L} \sim
...
$\posteriorsample_t$ y entrenando \textit{denoisers} para aproximar $\bE[
...
de la distribución multivariada (por ejemplo, agrupando las muestras en intervalos de valores de $\vy$) requiere
...
Por lo tanto, los \textit{denoisers} del proceso de muestreo posterior conceptual son iguales a
$\bE[\vx \mid \vx_t=\spcdot, \vy=\vnu]$, lo cual podemos aprender únicamente
a partir de muestras emparejadas $(\vx,
\vy)$,
y mediante la fórmula de Tweedie (\Cref{thm:tweedie}), podemos expresar estos \textit{denoisers} en
términos de la función de \textit{score} de $p_{\vx_t \mid \vy}$, la cual, según la regla de Bayes,
satisface
\begin{equation}
...
la convolución. ¡Esto no es más que la \textit{función de score incondicional}
obtenida del entrenamiento de difusión estándar que desarrollamos en la
\Cref{sub:compression_denoising}!
La función de \textit{score} condicional satisface entonces, para cualquier realización $(\vxi,
\vnu)$ de $(\vx_t, \vy)$,
\begin{equation}
...
\end{equation}
lo que da (según la fórmula de Tweedie) nuestros \textit{denoisers} propuestos como
\begin{align*}
...
\textit{denoiser} incondicional para la observación ruidosa, donde el término de corrección (el
...
operadores de gradiente en las funciones de \textit{score} anteriores para poder
...
disponemos de una expresión de forma cerrada
...
\begin{example}\label{example:denoising-conditional-gaussian}
    ...
    \begin{enumerate}
        ...
        \item Determinaremos la forma del término de coincidencia de la medición de la Ecuación
            \eqref{eqn:conditional-denoising-score}, lo que nos proporciona la corrección aditiva
            al \textit{denoiser} incondicional para $p_{\vx \mid
            \vx_t}$ que
            produce el \textit{denoiser} para $p_{\vx \mid \vx_t, \vy}$ (recuerde
    ...
    \begin{equation*}
        ...
        \end{bmatrix}.
    \end{equation*}
    Por independencia, se deduce que $(\vg, \vw)$ es gaussiana multivariada,
    ...
    \begin{equation*}
        ...
        \end{bmatrix}.
    \end{equation*}
    Ahora, aplicamos el hecho de que condicionar un vector aleatorio con distribución gaussiana multivariada
    ...
    \begin{equation}\label{eq:conditional-posterior-denoiser-gaussian-case}
        ...
        \right).
    \end{equation}
    La forma funcional de este \textit{denoiser} es bastante sencilla, pero presenta una
    ...
    \mid \vx_t}$ directamente utilizando la siguiente expresión para la distribución multivariada
    de $(\vx, \vx_t, \vy)$:
    \begin{equation}
        \begin{bmatrix}
            \vx \\
    ...
    Se trata, una vez más, de una distribución gaussiana multivariada; si nos limitamos únicamente a las dos últimas
    ...
    \textit{denoiser} posterior condicional
    ...
    \textit{denoiser} posterior incondicional \eqref{eq:posterior-denoiser-gaussian-case}
...
\begin{figure}[tbp]
    ...
    \caption{Simulación numérica de la configuración de muestreo condicional
        ...
        \textit{denoiser} posterior condicional exacto como el aproximado
        ...
        \textit{denoiser} de media a posteriori,
...
y que hemos recordado anteriormente, utilizando el verdadero \textit{denoiser} posterior condicional, es decir,
la \Cref{eq:conditional-posterior-denoiser-gaussian-case} (las muestras están marcadas con
círculos azules) y la
aproximación conveniente para este \textit{denoiser} realizada mediante la aproximación de
...
los \textit{denoisers} posteriores condicionales $\bE[\vx \mid \vy, \vx_t]$, mediante
\Cref{eq:posterior-sampling-denoiser-decomposition}:
\begin{equation}
    \bE[ \vx \mid \vx_t=\vxi, \vy=\vnu]
    \approx
...
entrenado como en la \Cref{sub:compression_denoising} para aproximar los \textit{denoiser} 
$\bE[\vx \mid \vx_t = \vxi]$ para cada $t \in [0, T]$, obtenemos los \textit{denoisers} 
...
solo se necesita
\begin{enumerate}
    \item Un \textit{denoiser} preentrenado $\bar{\vx}_{\theta}(t, \vxi)$ para la
...
\begin{algorithm}
    ...
    \textit{denoiser} incondicional y DPS.}
    ...
    \begin{algorithmic}[1]
        ...
        \Require{Un \textit{denoiser} incondicional \(\bar{\vx}_{\theta} \colon
...
para el
muestreo condicional de la distribución a posteriori $p_{\vx \mid \vy}$ dadas las mediciones,
siguiendo \eqref{eq:measurement-matching-observation}.
El \Cref{alg:iterative_denoising_conditional_DPS} describe este esquema
para el caso de
ruido de observación gaussiano con desviación estándar conocida $\sigma$, con ligeras
modificaciones para extenderlo a un proceso de ruido general, como en la
\Cref{eq:gen_additive_gaussian_noise_model} y la discusión que lo rodea en el
...
las mediciones
$\vy = h(\vx_o) + \vw$ son resultados de un proceso de medición conocido $h$ sobre
...
tenemos acceso a una distribución a priori de $p_{\vx}$, por ejemplo, de forma de un modelo de difusión.
...
el enfoque de modelado de difusión basado en la consistencia entre las mediciones de
\textcite{song2022solving} y la verdad de referencia (ground truth). Se puede observar que el método basado en difusión
...
su distribución multivariada $p(\z, \y)$ debería ser de dimensión extremadamente
baja. Por lo tanto, podemos combinar los dos códigos deseados, $\y$
y $\z$ e intentar entrenar un codificador combinado: 
\begin{equation}
    f: \x \mapsto (\z, \y)
\end{equation}
donde la distribución multivariada de $(\z, \y)$ es de muy baja dimensión.
...
Si bien un clasificador aprendido nos permite clasificar una imagen dada $\x$ en su
...
Por lo tanto, ahora suponemos únicamente que tenemos acceso a muestras de la distribución multivariada
de $(\vx, y)$:
\begin{equation}
    (\vx, y) \sim p_{\vx, y}.
\end{equation}
...
que se cumple la siguiente descomposición del \textit{denoiser} posterior condicional óptimo:
...
del \textit{denoiser} condicional óptimo es válida independientemente de si es válido el modelo de observación explícito
$\vy = h(\vx) + \vw$.
Esto significa que, si logramos aprender un \textit{denoiser} de este tipo
...
\textit{denoiser} incondicional aprendido para $\vx_t$ con parámetros $\theta_{\mathrm{d}}$ y
...
El lector debe tener en cuenta que el \textit{denoiser} condicional $\bar{\vx}_{\theta}$
...
clasificador con todo detalle.} y otro para los parámetros del \textit{denoiser} 
...
$\gamma > 0$
en la definición del \textit{denoiser} condicional ingenuo
\eqref{eq:conditional-denoising-mmse-denoiser-paired-data-approx},
refiriéndose al
\textit{denoiser} condicional resultante como si hubiera incorporado 
...
de la \Cref{eq:conditional-denoising-mmse-denoiser-paired-data}, ya que los
...
del \textit{denoiser} incondicional
...
necesario proporcionar la etiqueta de clase $y$ como una entrada adicional al \textit{denoiser} 
...
el \textit{denoiser} de mejor rendimiento en la práctica; ¡dicho \textit{denoiser} 
combina en realidad un \textit{denoiser condicional} para $\vx_t$ dado $y$ con una
...
\textit{denoiser} condicional
\eqref{eq:conditional-denoising-mmse-denoiser-paired-data} como una suma ponderada
de un \textit{denoiser} incondicional para $\vx_t$ con un término de corrección de la verosimilitud logarítmica
(con pesos posiblemente modificados, como en la guía del clasificador), aceptan la
aparente necesidad de entrenar un \textit{denoiser} condicional para $\vx_t$ dado $y$, tal como
...
\textit{denoiser} condicional con un \denoiser incondicional} para $\vx$ dado
$\vx_t$.\footnote{Dicho esto, \textcite{Ho2022-ry} propusieron en realidad utilizar
    una ponderación diferente a la que presentamos aquí, basándose en el hecho de que
    \textcite{Dhariwal2021-hg} sustituyeron de manera heurística el
    \textit{denoiser} incondicional de 
    \eqref{eq:conditional-denoising-mmse-denoiser-paired-data} por
    un \textit{denoiser} condicional. De hecho, la ponderación que derivamos y presentamos aquí
...
estructura, comenzamos con una versión «idealizada» del \textit{denoiser} con guía de clasificador
$\bar{\vx}_{\theta}^{\mathrm{CG}}$ definido en la
\eqref{eq:conditional-denoising-mmse-denoiser-paired-data-approx-cg},
para el cual el \textit{denoiser} $\bar{\vx}_{\theta_{\mathrm{d}}}$ y el clasificador
...
\Cref{eq:gen_tweedie}) para convertir entre funciones de \textit{score} y \textit{denoisers},
a fin de obtener
\begin{align*}
    \bar{\vx}_{\theta}^{\mathrm{CG,\,ideal}}(t, \vxi, \nu)
    &=
...
combinamos un \textit{denoiser} incondicional aprendido para $\vx$ dado $\vx_t$, como antes,
con un \textit{denoiser condicional} aprendido para $\vx$ dado $\vx_t$ y $y$.

Sin embargo, siguiendo a \textcite{Ho2022-ry} y la práctica habitual en el
entrenamiento de \textit{denoiser} de redes profundas,
lo habitual es utilizar la \textit{misma} red profunda para representar
tanto el \textit{denoiser} condicional como el incondicional mediante la introducción de una
etiqueta adicional, que denotaremos como $\varnothing$, para indicar el caso «incondicional».
Esto da lugar a la forma del \textit{denoiser} CFG:
...
\end{equation}
Para entrenar un \textit{denoiser} $\bar{\vx}_{\theta}(t, \vx_t, y^+)$ destinado a
el muestreo de guía sin clasificador, donde $y^+ \in
\set{1, \dots, K, \varnothing}$, procedemos de manera casi idéntica al
procedimiento de entrenamiento incondicional descrito en el \Cref{alg:learning_denoiser}, pero con dos
modificaciones:
\begin{enumerate}
    ...
    \item Cada vez que extraemos un par del conjunto de datos, obtenemos la etiqueta aumentada
        ...
        \begin{equation}
            y^+ =
            \begin{cases}
                \varnothing & \text{with probability } p_{\mathrm{incond}}; \\
                y & \text{else}.
            \end{cases}
        \end{equation}
        Aquí, $p_{\mathrm{incond}} \in [0, 1]$ es un nuevo hiperparámetro.
        Esto puede considerarse una forma de dropout \cite{srivastava2014dropout}.
\end{enumerate}
De esta manera, entrenamos un \textit{denoiser} condicional adecuado para su uso en el muestreo guiado sin clasificador.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Require{Un \textit{denoiser} \(\bar{\vx}_{\theta} \colon
...
\textcite{Ho2022-ry} presentan unos sólidos rendimientos empíricos en la generación de imágenes condicionadas por clase
con guía sin clasificador, y se han convertido en un pilar fundamental de
los modelos de difusión prácticos a mayor escala, como Stable Diffusion
\cite{rombach2022high} y sus derivados.

\paragraph{¿Qué \textit{denoisers} promueve aprender la guía sin clasificador?}
...
que abarca la parametrización y el entrenamiento del \textit{denoiser},
...
en el contexto simplificado de una distribución de datos de modelo de mezcla gaussiana y
\textit{denoiser}.
...
\begin{example}\label{example:denoising-gaussian-mixture-cfg}
    ...
    \end{equation}
    Por lo tanto, podemos expresar el \textit{denoiser} CFG con una fuerza de guía $\gamma
    > 1$ como
    ...
    \begin{enumerate}
        \item El primer término, correspondiente al \textit{denoiser incondicional},
            realiza el proceso de eliminación de ruido de la señal $\vx_t$ comparándola con un promedio de los
            \textit{denoisers} asociados a cada subespacio, ponderado por el grado de correlación
            que existe entre $\vx_t$ y cada subespacio.
        \item El segundo término, correspondiente al \textit{denoiser condicional}, 
            simplemente realiza el proceso de eliminación de ruido utilizando el \textit{denoiser}
    ...
    \begin{equation}\label{eq:cfg-denoiser-mog-low-rank-2}
        ...
        \end{split}
    \end{equation}
    donde combinamos el \textit{denoiser} condicional con el término correspondiente $k=y$
    ...
    \begin{enumerate}
        \item \textbf{Régimen bien correlacionado:} En este caso, supongamos que la entrada ruidosa
            ...
            sumando $k=y$ en el \textit{denoiser} incondicional es cercano a $1$.
            Entonces
            \begin{equation}
                \gamma + (1 - \gamma)
                \frac{\exp\rp{\frac{1}{2t^{2}(1
                ...
                \approx 1,
            \end{equation}
            todos los demás pesos son necesariamente cercanos a cero, y el \textit{denoiser} del CFG
            es aproximadamente igual al \textit{denoiser} asociado a la
            clase de condicionamiento
            $y$.

            Esto implica que: ¡\textit{si la entrada del \textit{denoiser} CFG está altamente
                correlacionada con uno de los subespacios de clase, el \textit{denoiser} es
                aproximadamente igual al \textit{denoiser} condicional de la clase
        ...
        \item \textbf{Régimen con baja correlación:} Por el contrario, si $\vx_t$
            ...
            \textit{denoiser} incondicional es cercano a $0$. Como resultado,
            \begin{equation}
                \gamma + (1 - \gamma)
                \frac{\exp\rp{\frac{1}{2t^{2}(1
                + t^{2})}\norm{\vU_{y}^{\top}\vx_t}_{2}^{2}}}{\sum_{i
            ...
            gran peso positivo
            al \textit{denoiser} asociado a $y$.
...
gaussianas, el \textit{denoiser} CFG presenta un fuerte sesgo hacia la
...
\textit{parametrización} del \textit{denoiser} en presencia de
...
\begin{example}\label{example:denoising-gaussian-mixture-cfg-parameterization}
    ...
    que el \textit{denoiser} CFG ideal toma la forma
    ...
    Ahora consideramos el problema de parametrizar un \textit{denoiser} aprendible
    $\vx_{\theta}^{\mathrm{CFG}}$ para representar el \textit{denoiser} óptimo
    ...
    general
    capaz de representar tanto los \textit{denoisers} incondicionales como los condicionales a la clase.
    ...
    \textit{denoiser} incondicional óptimo para $\vx_t$.
    Por otro lado,
    ...
    con media cero y covarianza identidad.
    Tenemos
    \begin{align*}
        \abs*{\vx_t^\top \vv_y}
        &=
    ...
    matrices identidad como covarianza, de la \textit{invarianza rotacional}
    ...
    grande,\footnote{Por ejemplo, basta considerar un régimen asintótico a gran escala,
    ...
    En otras palabras, \textbf{al evaluar el operador
        ...
        incrustación para
    una clase $\vv_{t, y}$ ¡se obtiene el \textit{denoiser} condicionado a la clase para $y$!}
    ...
    \eqref{eq:mog-conditional-denoising-unified-operator}
    ofrece una manera unificada de parametrizar los operadores
    que componen el \textit{denoiser} óptimo para $\vx$ \textit{dentro de una sola
    «red»}.
...
\textit{denoiser} ideal para observaciones ruidosas $\vx_t$ de $\vx$ cuando se utiliza
...
de la etiqueta de clase $y \mapsto \vv_y$ con el fin de implementar dicho \textit{denoiser}; tal y como
muestra el ejemplo, el \textit{denoiser} ideal es una suma ponderada de dichos operadores, con
pesos derivados de la intensidad de la guía $\gamma>1$.

Partiendo del marco descrito en el \Cref{ch:deep-networks}, que desarrolla arquitecturas de redes profundas
...
En la \Cref{sub:cfg} hemos visto, a nivel conceptual, cómo configurar los \textit{denoisers} 
...
Una arquitectura de capas conocida como de 'atención cruzada', cuya eficacia ha sido validada empíricamente
...
\begin{enumerate}
    ...
    \item \textit{Entrenamiento}: Esto implica la forma adecuada de utilizar nuestras muestras emparejadas
        $(\vx, \vy)$ para entrenar e implementar los \textit{denoisers} condicionales. El
        ...
        los \Cref{ch:lossy-compression,ch:consistent-representations}, junto con
...
\begin{equation}\label{eq:infomax-x-abstract}
    \text{Aprender} \enspace \vz = f(\vx) \enspace \text{tal que} \enspace I(\vz;
    \vy) \approx I(\vx; \vy).
\end{equation}
...
\begin{theorem}[Desigualdad del procesamiento de datos]\label{thm:dpi}
    ...
    \begin{proof}
        ...
        $(\vx, \vz)$. Dado que $\vz$ es independiente de $\vy$ condicionado a
        $\vx$, tenemos
        \begin{equation*}
...
al presentar
la información mutua en el \Cref{ch:lossy-compression}, la información mutua
...
de baja dimensión y tienen una entropía diferencial que se aproxima a $-\infty$ (por ejemplo, mediante
    la conexión con la función gaussiana de reducción de tasa que hemos estudiado en los
...
Esto hace que un criterio como el de la \Cref{eq:infomax-x} sea una base excelente para
...
señal de condicionamiento $y$ identificando qué componente específica de la mezcla genera
la observación, se podría construir una arquitectura flexible de \textit{denoiser} mediante
...
\vU_{\vx}$, «seleccionar» únicamente las incrustaciones relevantes para la componente de mezcla
...
ejemplos más específicos en el \Cref{ch:applications} (con detalles clave de implementación
...
En esta sección se ofrece una visión general de alto nivel sobre la generación de imágenes al estilo de Stable Diffusion.
Para obtener todos los detalles de implementación necesarios para volver a implementar
dicho sistema desde cero, consulte el \Cref{ch:applications}: en concreto, la
\Cref{sec:clm_text} describe el proceso de codificación de cadenas de texto en
una secuencia de identificadores enteros; la \Cref{sec:CLIP} describe el entrenamiento de los
codificadores de imagen-texto necesarios; y las \Cref{sec:auto-encode-img-gen,sec:cond_gen}
...
\begin{figure}[tbp]
    ...
    \caption{Esquema general del entrenamiento y la aplicación de un modelo generativo de texto a imagen,
        ...
        para un
        \textit{denoiser} condicional, entrenado según se describe algorítmicamente en la \Cref{sub:cfg} (véase la
        \Cref{sec:auto-encode-img-gen} para detalles de implementación). El codificador
        de texto puede estar
        preentrenado y congelado, o entrenarse conjuntamente con el \textit{denoiser}.
        \textbf{Derecha:}
        Al aplicar un modelo entrenado, se utiliza una indicación de texto deseada
        como condición
        y luego se realiza el muestreo con el modelo entrenado, tal como se muestra en el
        \Cref{alg:iterative_denoising_conditional_CFG}
...
de autocodificadores variacionales especializados y preentrenados $f : \bR^{D_{\mathrm{img}}} \to
...
\textcite{rombach2022high} implementan el condicionamiento de texto en el \textit{denoiser} utilizando
...
transformer causal --- véase la \Cref{sec:clm_text}), y sea $\psi : \bR^{d_{\mathrm{img}}}
\to \bR^{d_{\mathrm{model}} \times N_{\mathrm{img}}}$ la
correspondencia de una de
las representaciones intermedias en el \textit{denoiser}.\footnote{En la práctica,
    los \textit{denoisers} condicionados por texto añaden capas de atención cruzada a intervalos regulares
    dentro de la pasada hacia adelante del \textit{denoiser}, por lo que $\psi$ debe considerarse
...
En la arquitectura del \textit{denoiser} utilizada por \textcite{rombach2022high},
los bloques residuales de autoatención de la arquitectura del \textit{denoiser}, aplicados a la
...
Desde el punto de vista conceptual, este espacio de incrustación conjunto de texto e imagen y la propia capa de atención cruzada
presentan un gran parecido con el
\textit{denoiser} de mezcla condicional de gaussianas que derivamos en la sección anterior
...
preguntamos si aún podemos desarrollar un \textit{denoiser} para $\x$ con
...
$\vx$. Sin embargo, esto permite recuperar la componente de $\vx$
...
La \Cref{fig:projection-2D} ilustra esta relación
de manera concreta, mientras que la \Cref{fig:inference_distributed} ilustra el
...
para ella. Es decir, conocemos el \textit{denoiser}:
\begin{equation}
    \bE[\vy \mid \vy_t=\vnu] =
    \vnu + t^2 \nabla_{\vnu}\log p_t(\vnu).
    \label{eq:y-posterior-sampling-denoiser}
\end{equation}
O, incluso, podemos suponer que contamos con un número suficiente de
muestras de pares estéreo $(\y^0, \y^1)$ y que también hemos aprendido la
distribución multivariada de los pares. Haciendo un pequeño abuso de notación,
...
un \textit{denoiser} como el anterior).
...
la aplicación regrediente (pullback) del campo vectorial $t \nabla \log p_t(\y)$ bajo la
...
aprender la distribución de las escenas 3D, o al menos la distribución egocéntrica
de las escenas 3D. Sin embargo, las estructuras de baja dimensión de
la así llamada distribución aprendida contienen variaciones causadas por
...
vista, con respecto a algún sistema de coordenadas canónico de mundo. $\x^k$
...
$\vx$ a partir de $\vy$
utilizando el conocimiento previo sobre $\vx$ (es decir, la esparcidad).
En el contexto de la difusión con eliminación de ruido, tenemos acceso a una distribución a priori implícita para
$\vx$ a través de los \textit{denoisers} aprendidos $\bar{\vx}_{\theta}(t, \vxi)$. ¿Podemos
aún realizar
...
\end{equation*}
La parte dependiente de $\vx$ del lado derecho de la \Cref{eq:sure-risk} se denomina
estimador de riesgo no sesgado de Stein
(SURE). Si tomamos las expectativas sobre $\vx$ en la \Cref{eq:sure-risk},
observe que el lado derecho se puede escribir como una expectativa con respecto a $\vx_t$ --- en
particular, ¡el error cuadrático medio de \textit{cualquier denoiser $f$} se puede estimar
...
las expectativas sobre $\vx$ y se expresa la parte principal del lado derecho de la
...
una limitación de la aproximación DPS de la
...
\textit{denoiser} a posteriori $\bE[\vx \mid \vx_t=\vxi]$ (una matriz de gran tamaño). Esto se lleva a cabo
...
algoritmo DPS que hemos presentado en el
...
el aprendizaje conjunto de incrustaciones y CLIP en la \Cref{sub:text-cond}, constituye una base bien conocida e
...
\begin{exercise}[Corrección de la varianza posterior en el DPS]
    ...
    \begin{enumerate}
        \item Utilizando el código proporcionado en el GitHub del libro para implementar la
...
\begin{exercise}[Muestreo condicional en MNIST]
    \begin{enumerate}
        \item Entrene un clasificador sencillo para el conjunto de datos MNIST, utilizando
            una arquitectura
            de su elección. Además, entrene un \textit{denoiser} adecuado para su uso en
            muestreo condicional
            (\Cref{alg:iterative_denoising_conditional_CFG},
                ya que este \textit{denoiser} también se puede utilizar para la
            eliminación de ruido incondicional).
        \item Integre el clasificador en un muestreador condicional basado en
            guía por clasificador, tal como se describe en la primera parte de la
            \Cref{sub:cfg}.
        ...
        \item Integre el clasificador en un muestreador condicional basado en
            guía sin clasificador, tal como se describe en la \Cref{sub:cfg} y
