...
\Cref{ch:intro} (véase la \Cref{fig:autoencoder}), la {\em autocodificación} ---que integra los
...
el ICA y el aprendizaje de diccionarios esparsos comparten un objetivo similar.
...
autocodificadores--- encontrarlos mediante descenso por gradiente muchas veces resulta extremadamte no trivial.
...
\subsection{Auto-codificación esparsa}
...
la autocodificación suele realizarse en forma empírica. En situaciones más generales,
...
El autocodificador esparso está diseñado para resolver algunas de estas limitaciones. En
...
esparsas en el espacio de características. Por lo tanto, si incorporamos la esparcidad como medida
...
la autocodificación esparsa:
\begin{equation}
    \min_{f, g}
    [\|\Z\|_0 - \Delta R_{\epsilon}(\Z) + d(\X, \hat \X)],
    \label{eqn:autoencode-sparse}
\end{equation}
donde se sabe que la «norma» $\ell^0$ $\|\spcdot\|_0$ favorece la esparcidad.
Esto es muy similar al objetivo de reducción de la tasa de esparcidad
...
\begin{figure}
    ...
    \caption{Ilustración de un autocodificador esparso (SAE), en comparación con
    la de un autocodificador típico (AE) en la \Cref{fig:AE}. }
    \label{fig:SAE}
\end{figure}

Como método para aprender pares de autocodificación en forma integral, la autocodificación
...
la esparcidad en el autocodificador esparso o la gaussianidad en el VAE).
...
\begin{enumerate}
    ...
    \item La representación estructurada $\Z$ puede modelarse
        ...
        que podamos entrenar un \textit{denoiser} para acceder de manera eficiente a la
...
un \textit{denoiser} para acceder a su distribución, tal y como hemos estudiado en el
...
un \textit{denoiser} $\bar{\z}$ que elimine el ruido en forma iterativa en una muestra, pasando de la
distribución inicial a la distribución objetivo de las representaciones aprendidas.
Este \textit{denoiser} puede entrenarse utilizando los objetivos presentados en el \Cref{ch:denoising}
(véase también el \Cref{ch:conditional-inference} para los \textit{denoisers} condicionales). Por
ejemplo, consideremos el caso sencillo de flow matching (véanse el
    \Cref{exercise:implement_denoising_processes} y la
\Cref{sub:sampling_denoising}). Podemos definir el \textit{denoiser} como un
...
Cabe destacar la diferencia entre el entrenamiento de un \textit{denoiser} para las
...
eliminación de ruido sea potencialmente más desafiante. Por el contrario, las representaciones aprendidas
...
Una vez que hayamos entrenado el \textit{denoiser}, podemos utilizarlo para generar nuevas muestras a partir de
...
\begin{enumerate}
    ...
    \item Que sea \emph{autocontenido}.
