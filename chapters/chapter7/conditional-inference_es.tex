...
\subsection{Generación de imágenes condicionada por leyendas}\label{sub:text-cond}
En la \Cref{sub:cfg} hemos visto, a nivel conceptual, cómo configurar los denoisers
para utilizarlos con
\textit{la orientación sin clasificador}, la metodología práctica predominante para
...
Un diseño de capa de red neuronal que ha demostrado su eficacia empíricamente, conocido como «atención cruzada»,
...
\begin{enumerate}
    \item \textit{Desajuste de modalidades de entrada}: En las aplicaciones prácticas más interesantes,
        ...
        \textit{no se encuentran en espacios directamente comparables}, aunque comparten
        ...
        (denominada «leyenda»), donde $\vy$ correspondería a
...
únicamente en un pie de foto de alta calidad.
...
de una representación. La primera es que, en general, resulta computacionalmente difícil
...
\begin{align}
    ...
    \label{eq:clip-loss}
\end{align}
Desde la perspectiva de la maximización de la información mutua, CLIP es un sustituto manejable
...
\subsubsection{Aplicación a la generación de imágenes condicionadas por leyendas}
...
\Cref{sub:cfg}, con dos modificaciones clave: (i) La señal de condicionamiento es una
solicitud de texto tokenizada $\vY \in
\bR^{D_{\mathrm{text}} \times N}$, en lugar de una etiqueta de clase; (ii)
La eliminación del ruido de la imagen se
realiza en el espacio «latente» en lugar de sobre los píxeles sin procesar, utilizando un par especializado y
preentrenado de autoencodificadores variacionales $f : \bR^{D_{\mathrm{img}}} \to
...
\textcite{rombach2022high} implementa el condicionamiento de texto en el filtro de ruido utilizando
una capa conocida como «atención cruzada», inspirada en la arquitectura original de transformador codificador-decodificador
...
denote una red de codificación para las incrustaciones de texto (a menudo un
transformador causal---véase \Cref{sec:clm_text}), y sea $\psi : \bR^{d_{\mathrm{img}}}
\to \bR^{d_{\mathrm{model}} \times N_{\mathrm{img}}}$ denote la
correspondencia de a una de
...
en el transformador (que recordamos en detalle en \Cref{ch:applications}: véase
...
Obsérvese que, según la definición de la operación de autoatención, la atención cruzada
...
a los de \Cref{eq:vit-res-block} para el transformador de visión, van
...
similar a la del transformador.
...
Sin pérdida de generalidad, supongamos que $\vA$ tiene rango de fila completo,
...
A partir del proceso de eliminación de ruido de $\y_t$, tenemos
\begin{equation}
...
\end{equation}
De entre todos los $\x_{t-s}$ que satisfacen la restricción anterior,
elegimos arbitrariamente aquel que minimiza la distancia $\|\x_{t-s} -
...
conocidas?
Es posible que alguien se pregunte cuál es la razón para hacer esto: ¿por qué es
...
contienen información suficiente sobre la profundidad de la escena, desde el
punto de vista dado. Sin embargo, las imágenes 2D distan mucho de ser la representación más compacta
de una escena 3D, ya que una misma escena puede generar
un número infinito de imágenes 2D (altamente correlacionadas) o de pares de imágenes. De
...
\begin{remark}Cabe señalar que el objetivo anterior concuerda perfectamente con el
    ...
    admite un conjunto de movimientos tridimensionales de cuerpos rígidos que actúan sobre
...
Sin embargo, normalmente se desconoce el movimiento relativo exacto de nuestros ojos entre los distintos
puntos de vista. En términos más generales, también podría haber
...
conocimiento, este sigue siendo un problema abierto y complejo, probablemente como la
última frontera del problema de la visión 3D.
...
datos emparejados $(\vx, \vy)$---es decir, la verdad fundamental para cada observación $\vy$.
En muchos problemas inversos de relevancia práctica, esto no es así: uno de los
ejemplos más fundamentales se da en el contexto de la detección comprimida, que
...
únicamente datos con ruido: entre los ejemplos más destacados se incluyen el paradigma «noise2noise»
\cite{pmlr-v80-lehtinen18a}
y Ambient Diffusion \cite{daras2023ambient}.

Como curiosidad, cabe señalar que \Cref{eq:sure-risk} conduce a una prueba alternativa
...
ya que este campo está evolucionando rápidamente.
...
sirve a la vez como estudio de revisión y como prueba de rendimiento de varios métodos populares
...
relacionadas con
la optimización de objetivos de tipo Infomax a partir de muestras finitas.
...
\begin{exercise}[Muestreo condicional en MNIST]
    \begin{enumerate}
        ...
        \item Integre el clasificador en un muestreador condicional basado en
            la orientación del clasificador, tal como se describe en la primera parte de la
            \Cref{sub:cfg}.
        ...
        \item Integre el clasificador en un muestreador condicional basado en
            la orientación sin clasificador, tal como se describe en la \Cref{sub:cfg} y
