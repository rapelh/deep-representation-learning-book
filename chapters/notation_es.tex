...
\begin{itemize}
    ...
    \item Las matrices (ya sean deterministas o aleatorios) están en negritas 
        ...
        \(\vx_{i}\) es la \(i\a\) columna de \(\vX\), a no ser que se defina 
        de otra manera.
...
\begin{itemize}
    ...
    \item Para la reducción de ruido, la optimización y para procesos que tengan un
        índice de tiempo continuo, el tiempo casi siempre será un subíndice,
        p.ej., \(\vx_{t}\). En el caso de las secuencias discretas el índice podrá ser
...
\begin{itemize}
    \item Tal como en las secciones anteriores de notación de probabilidades y de aprendizaje automático,
        ...
        variables acentuadas «corrientes», p.ej. $\vx$.
    \item En los casos en que suponemos que los datos $\vx$ son generados por un modelo de entre
        ...
        acentuación corriente.\footnote{Esto podría surgir ya sea por haber hecho una 
        suposición acerca de los datos, o bien debido a que estos datos hayan sido generados
        sintéticamente. Thita convención nos permite adoptar una notación concisa tanto en
    ...
    \item Las \textit{variables de decisión} (o parametros aprendibles) del ajuste de un modelo 
        a unos datos observados se denotan con acentuación de «tilde», p.ej.\
        $\tilde{\vx}$, en casos donde existe un modelo subyacente $\vx$, o
        con tipografía corriente en casos donde no haya riesgos de confusión (p.ej., si
    ...
    \item Las \textit{soluciones óptimas} de problemas de optimización se denotan con
        ...
        $\vx_\star$ o $\vx^\star$).
    \item Los \textit{estimadores} para datos que correspondn a un modelo estadístico
    ...
    \item Las \textit{aproximaciones} asociadas a procedimientos computacionales del
        modelamiento de datos se denotan con acentuación de «sombrero», p.ej. $\hat{\vx}$ para
