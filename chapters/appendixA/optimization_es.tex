...
\begin{quote}
    ``{\em Dado que la estructura de todo el universo es perfecta y ha sido
        creada por el Creador sabio, no hay nada en el universo en
    lo que no se pueda percibir el sentido de un máximo o un mínimo.}''
...
damos pequeños pasos de forma iterativa, de modo que cada paso reduzca el valor de
la función \(\cL(\theta)\).
...
Esto requiere un poco más de rigor, lo cual abordaremos a continuación.
...
}, que generalizan el método del gradiente máximo a
objetivos no suaves.
...
enfoque de mayorizacón-minimización (es decir,
...
vista, parece que hemos cambiado un problema de minimización intratable por
...
\cite{schaipp2025surprising}, a pesar de que la optimización del aprendizaje profundo
...
quisieramos, podríamos utilizar el llamado \textit{descenso por gradiente por signos}:
...
descrito por estas iteraciones es el famoso optimizador \textit{Adam}
,\footnote{Para evitar errores de división por cero, dividimos por
...
\end{equation}
donde \(\eta_{k}\) es la tasa de aprendizaje establecida de forma adaptativa para cada parámetro.
...
En esta sección, presentamos los fundamentos de la \textit{diferenciación automática
} (AD o \textit{autodiff}), que es una
...
\begin{enumerate}
    \item Presentamos los \textbf{diferenciales}, un formalismo práctico
        ...
        tensores, etc.).
    \item Describimos los fundamentos de la diferenciación automática en \textbf{modo hacia adelante} y
...
\end{equation}
En los casos anteriores, \(\cL^{\prime}(\theta)\) era primero un
...
\begin{theorem}[Regla de la cadena diferencial]
...
La diferenciación automática en modo hacia adelante y en modo inverso son dos esquemas distintos para
...
\textit{modo hacia adelante de AD} calculará
\begin{equation}
...
único costo es multiplicar los jacobianos entre sí, el AD en modo hacia adelante tiene
...
\end{align}
En otras palabras, el algoritmo de sumas en modo hacia adelante tarda un tiempo de orden \(\cO(p(qr + rs))\),
...
de modo que \(f \colon \R^{d^{0}} \to \R^{d^{L}}\). Entonces, el modo hacia adelante
...
\begin{itemize}
    \item Si la función que se va a optimizar tiene \textit{más salidas que
        entradas} (es decir, \(d^{L} > d^{0}\)), utilice \textit{el AD en modo hacia adelante}.
...
Entonces, el AD en modo reverso nos indica que calculemos las derivadas en
...
de forma análoga al descenso por gradiente para funciones fuertemente convexas con
...
\begin{itemize}
    \item Tomemos \(T = 1\) (pasos de igual tamaño) y utilicemos actualizaciones para
        ...
        GD básico). Aquí, \textit{esperas} (pero no
        \textit{sabes}) que el optimizador pueda ajustar las tasas de aprendizaje
...
\end{itemize}
Por ejemplo, puedes utilizar esto al entrenar modelos de tipo CTRL (véase el
...
\begin{exercise}
    Realize el cálculo de retropropagación para un autoencoder
